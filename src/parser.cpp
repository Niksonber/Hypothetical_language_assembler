...
map<string, pair<string, int> > instructions = {
{"ADD", {"1", 2}},
{"SUB", {"2", 2}},
{"MULT",{"3", 2}},
{"DIV", {"4", 2}},
...
void erro_dire(string token, char section, int line){
	if (section=='t'){
		...
		if(token=="BEGIN" || token=="SPACE" || token=="EXTERN" || token=="PUBLIC"){
			cout << "semantico, diretiva na secao errada -> linha2: " << line << endl;
		}
	}
		...
		while(getline (code,line_string)){
			...
			if(tokens[0]=="SECTION"){
				if(tokens[1]=="TEXT") {
					...
					cout << "Secao desconhecida -> liha: "  << line << tokens[1] << endl;
			}
			int i=0;
			if(tokens[i].back()== ':'){
				...
				if(p!=-1){
					cout << "semantico, Erro simbolo redefinido -> linha: "<<  line << " " << tokens[i] << endl;	
				}else{
			...
			p = (search(tokens[i], instructions)).second;
			if(p!=-1){
				if (section == 'b' || section == 'd')
					cout << "semantico, secao errada -> linha: " << line << endl;
				position += p; 
			}else{
				p = search(tokens[i], directives);
				if(p!=-1){
					...
					cout << "Operacao nao identificada -> linha : " << line << " " << tokens[i] << endl;
				}
			}	
			line++;
		}
...
void erro(vector<string> tokens, int i, int line){
...
//SEGUNDA PASSAGEM
void second_pass(string filename, bool modular){
	int position = 0;
	int line = 1;
	...
	if(code.is_open()){
		while(getline (code,line_string)){
			...
			//procura instrucao
			p = search(tokens[i], instructions);
			if(p.second!=-1){
				position += p.second;
				relative << '0';
				if (p.second>0){
				...
				if (tokens.size() > i+1)
					semi_token = split(tokens[i+1], '+') ;
				if(tokens.size()==p.second + i || semi_token.size()>1){
					maq << string(p.first) << " ";
					if(tokens.size()>1){
						for(int j=i+1; j<tokens.size(); j++){
							try{
								...
								if(b.first!=-1){
									...
									maq << b.first << " ";
								}else if(semi_token.size()>1){
									b = search(semi_token[0]+":", TS);
									try{
										int tmpp = stoi(semi_token[1]);
										if(b.first!=-1 ){
										int us = search(tokens[j]+":", tab_uso);
										if(us!=-1)
											uso << tokens[j] << " " << position - (tokens.size() - (j-i)) << " ";
				...
				int b = search(tokens[i], directives);
				if(b!=-1){
